import { IndexPattern } from '../services/index-patterns';

export class VegaImportsSankeyChart {
  /**
   * Based on `[Logs] Source and Destination Sankey Chart`.
   * Changes:
   * - sortField must be modified to correctly order strings with different length (e.g use `pad(string, length)` on first part)
   *
   *  `datum.stack == 'stk1' ? pad(datum.stk1, 1000)+'::'+datum.stk2 : pad(datum.stk2, 1000)+'::'+datum.stk1`
   *
   * - key `{type: "formula", expr: "datum.stk1+'::'+datum.stk2", as: "key"}`
   * - fix colors
   *   - neither stk1 or stk2 can be used to define available colors
   *   - we must have a way to retrieve all unique stk1 (packageName) and stk2 (imported) values
   *     - index mapping with copy_to and asecond query can be used for this
   */
  static spec(repositoryName: string) {
    const query = {
      bool: {
        must: [
          '%dashboard_context-must_clause%',
          {
            term: { 'repository.name.keyword': repositoryName },
          },
        ],
    ...
    return {
      ...
      data: [
        {
          ...
          name: 'rawData',
          url: {
            index: IndexPattern.REPOSITORY_IMPORTS,
            body: {
              size: 0,
              query,
              aggs: {
        ...
        },
        {
          ...
          // From the result, take just the data we are interested in
          format: {
            property: 'aggregations.packages.buckets',
          },
          transform: [
            {
              type: 'formula',
        ...
        },
        {
          ...
          transform: [
            ...
            {
              type: 'formula',
              expr: "datum.stack == 'stk1' ? pad(datum.stk1, 1000)+'::'+datum.stk2 : pad(datum.stk2, 1000)+'::'+datum.stk1",
              as: 'sortField',
            },
      ...
      marks: [
        ...
        {
          // draw package name labels on the inner side of the stack
          type: 'text',
          from: {
            data: 'groups',
          },
